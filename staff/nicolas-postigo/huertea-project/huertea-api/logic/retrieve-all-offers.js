        ...
        .then((user) => {
            if (user) {
                ...
                return cursor.lean()
                    .then(_offers => {
                        if (_offers)
                            return _offers = _offers.map(({ _id, offername, titleoffer, image, price,offeraddress, phonecontact, emailcontact, owner }) => ({ id: _id.toHexString(), offername, titleoffer, image, price, offeraddress, phonecontact, emailcontact, owner }))
                        else throw new NotFoundError('there are no offers to retrieve')
                    });

            } else throw new NotFoundError(`the offer with id ${ownerId} was not found`);
        });
} 
