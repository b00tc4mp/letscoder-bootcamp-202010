module.exports = {
    handleRegisterUser: require('./handle-register-user'),
    handleAuthenticateUser: require('./handle-authenticate-user'),
    handleRetrieveUser: require('./handle-retrieve-user'),
    handleEditUser: require('./handle-edit-user'),
    handleSearchArtists: require('./handle-search-artists'),
    handleSaveImage: require('./handle-save-image'),
    handleRetrieveUserImage: require('./handle-retrieve-user-image'),
    handleSaveLive: require('./handle-save-live'),
    handleSaveLiveImage: require('./handle-save-live-image'),
    handleRetrieveLiveImage: require('./handle-retrieve-live-image'),
    handleRetrieveLives: require('./handle-retrieve-lives'),
    handleModifyLive: require('./handle-modify-live')
}